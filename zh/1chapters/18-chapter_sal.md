...
为了适配更多的网络协议栈类型，避免系统对单一网络协议栈的依赖，RT-Thread 系统提供 SAL（套接字抽象层）组件，该组件完成对不同网络协议栈接口的抽象并对上层提供一组标准的 POSIX 网络编程接口，这样开发者只需要关心和使用网络应用层提供的 socket 接口，无需关心底层使用具体网络协议栈类型，极大的提过的系统的兼容性，方便开发者使用。
...
RT-Thread 的网络应用层提供的接口主要以 POSIX 标准接口为主，这样也能够保证程序可以在PC 上编写、调试，然后再移植到RT-Thread 操作系统上。
...
上述为标准POSIX接口中 socket 函数定义，其中 domain 表示协议域又称为协议簇（family），用于判断使用哪种协议簇，常见的有 AF_INET、AF_INET6 等。 SAL 组件中就是通过判断 socket 创建时传入的协议簇类型，定位该 socket 使用的协议栈类型。目前支持的如下两种协议栈：

- lwIP 协议栈： family = AF_INET、sec_family = AF_INET

- AT Socket 协议栈： family = AF_AT、sec_family = AF_INET

 AT socket 协议栈是 RT-Thread 系统自主研发的基于 AT 组件的简单网络协议栈，其设备的连接和数据的通讯都是通过 AT 命令完成，支持 POSIX 标准。
